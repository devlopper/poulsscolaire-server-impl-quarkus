...
/**
 * Cette classe représente la transmission de {@link Budget}.
 *
 * @author Christian
 *
 */
@ApplicationScoped
public class BudgetTransmitBusiness extends AbstractIdentifiableUpdateBusiness<Budget,
  ...
  protected void validate(ByIdentifierRequestDto request, StringList messages, Budget processing) {
    ...
    validator.validateStatusChange(processing.status, BudgetStatus.TRANSMITTED, messages);
  }

  @Override
  protected void prepare(Budget processing, ByIdentifierRequestDto request) {
    super.prepare(processing, request);
    processing.status = BudgetStatus.TRANSMITTED;
  }
  ...
  protected String getActionName() {
    return BudgetStatus.TRANSMITTED.getActionName();
  }

  @Override
  protected String getActionDone() {
    return BudgetStatus.TRANSMITTED.getName();
  }
}
