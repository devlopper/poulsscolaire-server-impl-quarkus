...
class BudgetServiceImplTest extends AbstractTest {
  ...
  @Test
  void create() {
    BudgetCreateBusiness business =
        installMockForType(BudgetCreateBusiness.class);
    Mockito.when(business.process(any())).thenReturn(new CreateResponseDto());

    RestAssured.given().contentType(ContentType.JSON).accept(ContentType.JSON).when()
        .post(BudgetService.PATH + "/" + BudgetService.CREATE_PATH).then()
        .log().ifError().statusCode(jakarta.ws.rs.core.Response.Status.CREATED.getStatusCode());
  }

  @Test
  void readMany() {
    BudgetReadManyBusiness business =
        installMockForType(BudgetReadManyBusiness.class);
    Mockito.when(business.process(any())).thenReturn(new BudgetGetManyResponseDto());

    RestAssured.given().contentType(ContentType.JSON).accept(ContentType.JSON).when()
        .post(BudgetService.PATH + "/" + BudgetService.GET_MANY_PATH)
        .then().log().ifError().statusCode(jakarta.ws.rs.core.Response.Status.OK.getStatusCode());
  }

  @Test
  void readOne() {
    BudgetReadOneBusiness business =
        installMockForType(BudgetReadOneBusiness.class);
    Mockito.when(business.process(any())).thenReturn(new BudgetDto());

    RestAssured.given().contentType(ContentType.JSON).accept(ContentType.JSON).when()
        .post(BudgetService.PATH + "/" + BudgetService.GET_ONE_PATH)
        .then().log().ifError().statusCode(jakarta.ws.rs.core.Response.Status.OK.getStatusCode());
  }
  ...
  void readByIdentifier() {
    ...
    RestAssured.given().contentType(ContentType.JSON).accept(ContentType.JSON).when()
        .post(BudgetService.PATH + "/"
            + BudgetService.GET_BY_IDENTIFIER_PATH)
        .then().log().ifError().statusCode(jakarta.ws.rs.core.Response.Status.OK.getStatusCode());
  }

  @Test
  void update() {
    BudgetUpdateBusiness business =
        installMockForType(BudgetUpdateBusiness.class);
    Mockito.when(business.process(any())).thenReturn(new IdentifiableResponseDto());

    RestAssured.given().contentType(ContentType.JSON).accept(ContentType.JSON).when()
        .put(BudgetService.PATH + "/" + BudgetService.UPDATE_PATH).then()
        .log().ifError().statusCode(jakarta.ws.rs.core.Response.Status.OK.getStatusCode());
  }
  
  @Test
  void delete() {
    BudgetDeleteBusiness business =
        installMockForType(BudgetDeleteBusiness.class);
    Mockito.when(business.process(any())).thenReturn(new IdentifiableResponseDto());

    RestAssured.given().contentType(ContentType.JSON).accept(ContentType.JSON).when()
        .delete(BudgetService.PATH + "/" + BudgetService.DELETE_PATH)
        .then().log().ifError().statusCode(jakarta.ws.rs.core.Response.Status.OK.getStatusCode());
  }
