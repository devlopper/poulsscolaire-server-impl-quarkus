...
@QuarkusTest
class DeadlineGroupDynamicQueryTest {

  @Inject
  DeadlineGroupDynamicQuery dynamicQuery;

  DynamicQueryParameters<DeadlineGroup> parameters = new DynamicQueryParameters<>();

  @Test
  void buildQueryString() {
    parameters.setResultMode(ResultMode.ONE);
    parameters.filter().addCriteria(DeadlineGroupDto.JSON_IDENTIFIER, "1");
    assertEquals("SELECT t.identifier FROM DeadlineGroup t WHERE t.identifier = :identifiant",
